...
# ==== フォルダ設定(設定済み。変えたいときはここのURLを書き換える)====
DEFAULT_SOURCE_FOLDER = "https://drive.google.com/drive/folders/15qwtydkXB0OYdopFPVyzPbzN2eEulXOE"  # 監視(よちあい)
DEFAULT_OUTPUT_FOLDER = "https://drive.google.com/drive/folders/1nTp2jHx0MZLCJLKUJodWEfFToPnNU0wP"  # 保存先
...
def source_folder():
    return id_of(os.environ.get("SOURCE_FOLDER") or DEFAULT_SOURCE_FOLDER)


def output_folder():
    return id_of(os.environ.get("OUTPUT_FOLDER") or DEFAULT_OUTPUT_FOLDER)
...
def drive():
    ...
    if sa:
        ...
        # 鍵不要方式: Workload Identity連携(google-github-actions/authが用意した認証を使う)
        import google.auth
...
def upload_image(svc, image, name, cv2, tmp):
    ...
    with_retry(lambda: svc.files().create(
        body={"name": name, "parents": [output_folder()]},
        media_body=media,
        supportsAllDrives=True,
    ).execute(), "画像のアップロード")
